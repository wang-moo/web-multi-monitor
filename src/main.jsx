...
const DEFAULT_URL = "https://dev-h5-hall-sz.gameh5pro.com/index.html";
const PAGE_SIZE = 4;
const SESSION_STORAGE_KEY = "web-multi-monitor:last-session";
...
function readSavedSession() {
  const empty = { restoreOnStartup: true, instances: [] };
  try {
    const saved = JSON.parse(localStorage.getItem(SESSION_STORAGE_KEY));
    if (!saved || !Array.isArray(saved.instances)) return empty;
    return {
      restoreOnStartup: saved.restoreOnStartup !== false,
      instances: saved.instances
        .filter(({ url, speed }) => typeof url === "string" && Number.isFinite(speed) && speed > 0)
        .map((settings) => ({
          ...settings,
          deviceId: /^[0-9a-f]{32}$/i.test(settings.deviceId) ? settings.deviceId.toLowerCase() : newDeviceId(),
        })),
    };
  } catch {
    return empty;
...
function App() {
  const savedSession = useRef(readSavedSession()).current;
  const [targetUrl, setTargetUrl] = useState(DEFAULT_URL);
  const [instances, setInstances] = useState([]);
  const [instanceSettings, setInstanceSettings] = useState({});
  const [restoreOnStartup, setRestoreOnStartup] = useState(savedSession.restoreOnStartup);
  ...
  const layoutSync = useMemo(() => createLayoutSync(
    (layout) => invoke("set_instance_bounds", layout), setError,
  ), []);
  ...
  useEffect(() => {
    ...
    async function initializeSession() {
      ...
      const failures = [];
      try {
        let labels = await loadInstances();
        if (IS_TAURI && labels.length === 0 && savedSession.restoreOnStartup) {
          for (const settings of savedSession.instances) {
            try {
              const label = await invoke("create_instance", { url: settings.url, speed: settings.speed, deviceId: settings.deviceId });
              restored[label] = settings;
            } catch (reason) {
              failures.push(String(reason));
            }
          }
          labels = await loadInstances();
        }

        setInstanceSettings(restored);
        setSpeeds(Object.fromEntries(labels.map((label) => [label, restored[label]?.speed ?? 1])));
        setSpeedDrafts(Object.fromEntries(labels.map((label) => [label, String(restored[label]?.speed ?? 1)])));
        setRotations(Object.fromEntries(labels.map((label) => [label, restored[label]?.rotation ?? 0])));
        if (failures.length) setError(`${failures.length} 个历史窗口恢复失败：${failures[0]}`);
      } catch (reason) {
    ...
    initializeSession();
  }, [loadInstances, savedSession]);

  useEffect(() => {
    if (!sessionReady) return;
    localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify({
      restoreOnStartup,
      instances: instances.flatMap((label) => {
        const settings = instanceSettings[label];
        return settings ? [{
          url: settings.url,
          deviceId: settings.deviceId,
          speed: speeds[label] ?? settings.speed,
          rotation: rotations[label] ?? settings.rotation ?? 0,
        }] : [];
      }),
    }));
  }, [instanceSettings, instances, restoreOnStartup, rotations, sessionReady, speeds]);
  ...
  const syncLayout = useCallback(() => {
    ...
    for (const label of instances) {
      ...
      if (!visible.has(label) || !host) {
        layouts.push({ label, x: -1, y: -1, width: 1, height: 1, visible: true, rotation: rotations[label] ?? 0 });
        continue;
      }
    ...
    layoutSync(layouts);
  }, [instances, pageItems, rotations, layoutSync]);
  ...
  async function createOne(event) {
    ...
    try {
      ...
      const label = IS_TAURI ? await invoke("create_instance", { url: parsed.href, speed: globalSpeed, deviceId }) : `instance-${instances.length + 1}`;
      ...
      setInstanceSettings((current) => ({ ...current, [label]: { url: parsed.href, deviceId, speed: globalSpeed, rotation: 0 } }));
      setMaximized(null);
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        <aside className="sidebar">
          ...
          <section className="side-section session-settings">
            <h2>启动设置</h2>
            <label>
              <input type="checkbox" checked={restoreOnStartup} onChange={(event) => setRestoreOnStartup(event.target.checked)} />
              <span><b>恢复上次会话</b><small>重新启动时恢复未关闭的窗口、速度与旋转角度</small></span>
            </label>
          </section>
                ...
                <article className="monitor-tile" key={label}>
                  <header className="tile-header">
                    <div className="tile-info">
                      <div className="tile-title"><span className="online-dot" /><b>隔离实例 {instanceNumber(label)}</b><small title={`设备号：${instanceSettings[label]?.deviceId ?? "未记录"}`}>设备 …{instanceSettings[label]?.deviceId?.slice(-8) ?? "未记录"} · {rotations[label] ?? 0}°</small></div>
