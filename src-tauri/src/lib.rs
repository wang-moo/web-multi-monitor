...
struct AppState {
    ...
    speeds: Mutex<HashMap<String, Arc<AtomicU64>>>,
}
    ...
    Ok(format!(
        r#"(() => {{
          ...
          const apply = () => {{
            ...
            style.textContent = `html {{ overflow: hidden !important; }} body {{ position: fixed !important; inset: 0 !important; width: 100vw !important; height: 100vh !important; overflow: hidden !important; transform-origin: center center !important; transform: rotate({rotation}deg) scale({scale}) !important; }}`;
          }};
          apply();
        }})();"#
    ))
    ...
    Ok(format!(
        r#"(() => {{
          ...
          const apply = (value) => {{
            ...
            if (scheduler?.setTimeScale) scheduler.setTimeScale(value);
          }};
...
async fn create_instance(
    ...
    let device_id = instance_device_id(state.device_seed, id);
    ...
    let speed_state = Arc::new(AtomicU64::new(speed.to_bits()));
    ...
    let webview = WebviewBuilder::new(&label, WebviewUrl::External(url))
        ...
        .on_page_load(move |webview, payload| {
            if payload.event() == PageLoadEvent::Finished {
                let speed = f64::from_bits(page_speed.load(Ordering::Relaxed));
